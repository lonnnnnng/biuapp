...
- 审计日期：2026-08-08
- Git 分支：`main`
- Android 版本：`0.1.18` 工作树，包含 M11-M17 及 M18 首页聚合、搜索分页、快捷队列、拖动/批量编辑、睡眠定时和统一历史纵向切片
...
Android 端的“找歌 -> 用户/空间发现 -> 关注视频动态 -> 单 P/多 P 音视频播放 -> 播放模式和倍速 -> 队列编辑 -> 后台和锁屏控制 -> 下载 -> 状态恢复”主链路已经比较完整；当前主要缺口集中在统一搜索、歌词高级调整和更深的外观个性化，暂时不能算功能完全对齐。
...
- 综合热门、每周必看、全站排行、入站必刷、视频搜索和自选 UP 首页；配置来源后支持最新发布、未播放、最近播放，并可按全部、本地分组或单个 UP 筛选和连续加载。
...
- 统一搜索歌曲视频、UP 主、系列/合集和本地歌单，并补充搜索历史与排序。
