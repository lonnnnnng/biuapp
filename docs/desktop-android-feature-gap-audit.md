...
- 审计日期：2026-08-08
- Git 分支：`main`
- Android 版本：`0.1.18` 工作树，包含 M11-M17 及 M18 搜索分页、快捷队列、拖动/批量编辑和睡眠定时纵向切片
...
Android 端的“找歌 -> 用户/空间发现 -> 关注视频动态 -> 单 P/多 P 音视频播放 -> 播放模式和倍速 -> 队列编辑 -> 后台和锁屏控制 -> 下载 -> 状态恢复”主链路已经比较完整，但关注分组和深度个性化仍明显落后于桌面端，暂时不能算功能完全对齐。

当前 Android 端包含“推荐”“动态”和“账号”三个主页面。账号音乐库包含收藏夹、统一历史、本地音乐和下载；历史页内部按全部/在线/本地来源筛选。

## 已经基本对齐

- Bilibili H5 登录与账号状态识别。
- 综合热门、每周必看、全站排行、入站必刷、视频搜索和自选关注 UP 首页；默认来源与每个 UP Tab 均支持下拉刷新和上滑续载。
...
| --- | --- | --- |
| 关注列表 | UP 主中心支持独立关注列表、分页、本地筛选、关注/取消关注，并与首页 UP 范围同步；Redmi 真实账号已验收 | 关注分组管理 |
