...
- 审计日期：2026-08-01
- Git 分支：`main`
- Android 版本：`0.1.14` 工作树，包含 M11 用户/关注、M12 动态互动、M12.1 热门分页、M12.2 推荐 Tab/刷新和 M13 显示布局改动
...
- 播放队列设为下一首、移除单项和清空，编辑后的队列可跨进程恢复。
...
| 播放队列 | 支持查看、点击切换、设为下一首、移除、清空和持久化恢复 | 任意拖动排序 |
...
`feature-migration-matrix.md` 和 `development-roadmap.md` 已同步稍后再看产品决策、“我收藏的”与视频合集、下载、队列持久化、播放模式、倍速和队列编辑状态。后续每完成一个纵向切片仍需同步维护三份文档，避免计划状态与实际代码再次分离。
