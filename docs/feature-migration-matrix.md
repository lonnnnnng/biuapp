...
| 热门推荐、自选 UP 首页、搜索 | Compose 列表 + 热门/每周必看/排行 API + UP 投稿 WBI | M2/M12.1 | 综合热门服务端分页、每周必看按期续载、全站排行客户端分批及自选 UP 独立分页已完成；视频搜索保持可用 |
...
| 关注、动态、用户空间 | UP 主中心 + 动态主页面 | M11/M12 | 用户搜索、空间投稿、关注切换、视频动态游标分页、播放、点赞/取消点赞及确认三连已实现；真实点赞往返已验收，三连真实扣币待授权，关注分组待后续阶段 |
