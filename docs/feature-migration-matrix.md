...
| 热门推荐、自选 UP 首页、搜索 | Compose 列表 + 热门/每周必看/排行/入站必刷 API + UP 投稿 WBI | M2/M12.1/M12.2 | 四个默认来源和每 UP 独立 Tab 已完成；支持下拉刷新、上滑续载和视频搜索，固定列表不发送伪分页参数 |
...
| 在线历史、本地历史 | 账号 API + Room 本地历史 | M3/M8 | 在线分页、搜索、删除、清空、进度上报及本地续播已完成 |
| 稍后再看 | 保留 API，不提供 Android 页面入口 | - | 产品决策为不迁移入口 |
| 关注、动态、用户空间 | UP 主中心 + 动态主页面 | M11/M12/M12.2 | 用户搜索、空间投稿、关注切换、视频动态下拉刷新与游标分页、播放、点赞/取消点赞及确认三连已实现；真实点赞往返已验收，三连真实扣币待授权，关注分组待后续阶段 |
