...
DASH URL 可能过期。播放失败且响应符合链接失效特征时，解析器必须按 `bvid + cid` 重新请求地址，并限制单曲自动重试次数，防止无限循环。播放服务为 ExoPlayer 启用 `WAKE_MODE_NETWORK`；当屏幕关闭且播放意图仍为继续时，由播放器持有 CPU 与 Wi-Fi 锁，确保当前分 P 自然结束后仍能准备并自动播放下一分 P，且老设备不会因熄屏省电挂起下一 P 的网络请求。前台媒体服务只保证进程与通知存活，不能替代播放线程和网络链路所需的唤醒锁。
