...
> 当前稳定版：**0.1.28**。BiuApp 的主线是“听歌”，视频播放是手动切换的补充模式；它不是完整的 Bilibili 社区客户端。
...
2. 下载当前稳定版 `BiuApp-v0.1.28.apk`，或选择最新版本中的同名 APK 资产。
...
- APK SHA-256：待 `v0.1.28` 正式包生成后回填（仓库成员可在同一 Release 下载 `SHA256SUMS` 核对）
...
`0.1.28` 修复应用冷启动自动播放：冷启动只恢复队列、当前分 P 和进度，首次发声必须由用户手动点击播放；用户开始播放后，多 P 自动转场和后台续播逻辑保持不变。工程门禁已通过，完整真机验收待用户自行完成；正式包签名、对齐、版本和 SHA-256 将在发布后以 `v0.1.28` Release 为准。
